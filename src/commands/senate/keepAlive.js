...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('keepalive')
    .setDescription(
      'Enable or disable keep-alive service for a thread (prevents threads from being auto-archived).'
    )
  ...
  async execute(i) {
    ...
    if (
      !i.member.roles.cache.has(config.roles.guildOfficer) &&
      !i.member.roles.cache.has(process.env.OWNER)
    )
    ...
    if (sub === 'enable' && thread) {
      return i.editReply(
        'Keep-alive is already enabled for this thread. To disable, use `/thread keep-alive disable`.'
      );
    }
    ...
    if (sub === 'enable' && !thread) {
      ...
      return i.editReply(
        'Keep-alive is now enabled for this thread. To disable, use `/thread keep-alive disable`.'
      );
    }
    ...
    if (sub === 'disable' && thread) {
      ...
      return i.editReply(
        'Keep-alive is now disabled for this thread. To re-enable, use `/thread keep-alive enable`.'
      );
    }

    if (sub === 'disable' && !thread) {
      return i.editReply(
        'Keep-alive was not enabled for this thread. To enable, use `/thread keep-alive enable`.'
      );
    }
  },
};
