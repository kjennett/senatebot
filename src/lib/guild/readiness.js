...
function KAMReadiness (ggAccountData) {
    ...
    for (let i = 0; i < ggAccountData.units.length; i++) {
        // Shaak 501st check
        if (SHAAK_TROOPER_IDS.includes(ggAccountData.units[i].data.base_id)) {
            // Must exceed minimum power requirement
            if (ggAccountData.units[i].data.power < KAM_MINIMUM_POWER) {
            ...
            else if (ggAccountData.units[i].data.relic_tier - 2 < KAM_RECOMMENDED_RELIC) shaakTroopers.push(Readiness.MAYBE_READY);
        ...
        // Bad Batch check
        if (BAD_BATCH_IDS.includes(ggAccountData.units[i].data.base_id)) {
            if (ggAccountData.units[i].data.power < KAM_MINIMUM_POWER) {
                // Must exceed minimum power requirement
            ...
            // Not at recommended relic, but enough power to attempt.
            else if (ggAccountData.units[i].data.relic_tier - 2 < KAM_RECOMMENDED_RELIC) badBatch.push(Readiness.MAYBE_READY);
            // Exceeds recommended minimum relic level and has enough power to attempt mission
...
function WatReadiness (ggAccountData) {
    ...
    for (let i = 0; i < ggAccountData.units.length; i++) {
        if (GEO_IDS.includes(ggAccountData.units[i].data.base_id)) {
            ...
            else if (ggAccountData.units[i].data.gear_level < WAT_RECOMMENDED_GEAR) geos.push(Readiness.MAYBE_READY);
...
function RevaReadiness (ggAccountData) {
    ...
    for (let i = 0; i < ggAccountData.units.length; i++) {        
        ...
        if (!GIPass && ggAccountData.units[i].data.base_id === GI) {
            ...
            if (ggAccountData.units[i].data.relic_tier - 2 < REVA_REQUIRED_RELIC) return Readiness.NOT_READY;
        ...
        // Inquisitor check
        if (INQUISITOR_IDS.includes(ggAccountData.units[i].data.base_id)) {
            inquisitorsSeen++; // increase number seen

            // if above R7, increase passing counter
            if (ggAccountData.units[i].data.relic_tier - 2 >= REVA_REQUIRED_RELIC) inquisitorsPass++; 
...
function GetReadinessFunction (character) {
    switch (character) {
        case 'Wat Tambor': return WatReadiness;
        case 'Ki-Adi-Mundi': return KAMReadiness;
        case 'Third Sister': return RevaReadiness;
        default: return null;
    }
}
